  ...
  zh: {
    privacy: {
      ...
      sections: [
        ...
        { id: "agent-tasks", title: "背景智能體任務", body: ["啟用後，智能體任務是一項獨立的登入服務。開始任務會將目標、參考資料、所選智能體設定、追加指示、進度、模型與工具訊息及產出檔案儲存至帳號，並依任務需要經固定的 Power Champion 閘道傳送相關內容給所選模型。重新整理或關閉頁面後，任務紀錄仍會保留，每個帳號最多保留 50 個任務。為讓任務在關閉頁面後繼續執行，你明確提供的模型 API 金鑰會在伺服器加密保存；任務完成、失敗或取消時，會清除目前儲存的加密金鑰，暫停的任務則會保留以供繼續。受存取控制的備份可能保留較早的紀錄。金鑰不會存入瀏覽器儲存空間。讀取公開網頁前須批准確切網址，請求不會向該網站傳送帳號 Cookie 或模型金鑰。帳號資料刪除需求請聯繫支援。"] },
